...
for ii=1:nframe,
  ...
  l=get_segment_label( label, seg);%获取分割块标签
  L_all{ii}=l';%分割超像素的标签，L_all{ii}为行向量

  load([basefeatdir '/' id '.mat']);

    f_g=gkdes';%f_g为一个nseg*nfeature的矩阵，其中nseg为分割超像素数，nfeature为特征维数
    f_r=rgbkdes';
    f_gd=gkdes_depth';
    f_lb=lbpkdes';
%     f_s=spinkdes';

  wseg0 = get_kdes_weight_seg(seg0, 2, 16 );%为一个列向量，每一项代表一个分割超像素的权值
  f_1 = features_merge_segments( [f_g f_r f_gd], seg0, wseg0, seg );
  wseg0 = get_kdes_weight_seg(seg0, 2, 16 );%为一个列向量，每一项代表一个分割超像素的权值
  f_2 = features_merge_segments( f_lb , seg0, wseg0, seg );
